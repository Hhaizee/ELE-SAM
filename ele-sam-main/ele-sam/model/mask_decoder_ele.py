...
class LCE(nn.Module):
    def __init__(self, transformer_dim):
        super().__init__()
        self.c_conv = nn.Conv1d(transformer_dim, transformer_dim, kernel_size=3, padding=1)
        self.norm = nn.LayerNorm(transformer_dim)

    def forward(self, queries):
        c = self.c_conv(queries.transpose(1, 2)).transpose(1, 2)
        return self.norm(c + queries)
...
class PromptAdapater(nn.Module):
    ...
    def __init__(
        ...
        self.LCE = LCE(embedding_dim)

    def forward(
            self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor, early_embeddings: Tensor,
            final_embeddings: Tensor, prompt_encoder, image_record, guiding_embedding, v_coarse, n_sample_points=4
    ) -> Tuple[Tensor, Tensor]:

        queries = self.LCE(queries)
        ...
        attn_out, v_refined = self.cross_attn_token_to_image.forward_return_v(q=queries + query_pe,
                                                                              k=guiding_embedding + key_pe,
                                                                              v=guiding_embedding)
        queries = attn_out
        ...
        final_mask = (unceratinty_map_norm >= 0.5).detach() * refined_mask + (
                    unceratinty_map_norm < 0.5).detach() * coarse_mask
...
class MaskDecoderELE(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.hf_mlp_hr = MLP(transformer_dim, transformer_dim, transformer_dim // 16, 3)
    ...
    def forward(
        ...
        """
        # sparse_prompt_embeddings = self.ModalAligner(image_embeddings).unsqueeze(0)

        early_embeddings = interm_embeddings[0].permute(0, 3, 1,
                                                        2)  # early-layer ViT feature, after 1st global attention block in ViT

        fusion_feature = self.embedding_encoder(image_embeddings) + self.compress_vit_feat(early_embeddings)
        final_embeddings = fusion_feature
        ...
        for i_batch in range(batch_len):
            ...
            mask, mask_hr, iou_pred, interm_result = self.predict_masks(
        ...
        return masks_sam, masks_hr, iou_preds, uncertain_maps, final_masks, coarse_masks, refined_masks, box_preds
    ...
    def predict_masks(
        ...
        # Run the transformer
        prompt_adapter_args = {
            "early_embeddings": early_embeddings, "final_embeddings": final_embeddings, "image_record": image_record,
            "prompt_encoder": prompt_encoder, "prompt_adapter": self.prompt_adapter,
            "guiding_embedding": guiding_embedding}
        ...
        upscaled_embedding_ours = self.embedding_maskfeature(upscaled_embedding_sam) + final_embeddings.repeat(b, 1, 1,
                                                                                                               1)  # (1,32,256,256)

        upscaled_embedding_sam = self.embedding_maskfeature(upscaled_embedding_sam)
        upscaled_embedding_ours = self.embedding_maskfeature(upscaled_embedding_ours)
        ...
        masks = mask * masks_ours + mask

        upscaled_embedding_ours = self.output_upscaling_hr(upscaled_embedding_ours)

        hyper_in_hr = self.hf_mlp_hr(mask_tokens_out[:, 4, :])
        b, c, h, w = upscaled_embedding_ours.shape
        masks_hr = (hyper_in_hr @ upscaled_embedding_ours.view(b, c, h * w)).view(b, -1, h, w)

        iou_pred = self.iou_prediction_head(iou_token_out)

        return masks, masks_hr, iou_pred, Interm_result
